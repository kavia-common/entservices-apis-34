...
namespace ProxyStubs {
    ...
    ProxyStub::MethodHandler CompositPluginStubMethods[] = {
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_4ffa4955Implementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_4ffa4955 = nullptr;
            ProxyStub::UnknownProxy* parameter_4ffa4955Proxy = nullptr;
            if (parameter_4ffa4955Implementation != 0) {
                parameter_4ffa4955Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_4ffa4955Implementation, false, _parameter_4ffa4955);

                ASSERT((_parameter_4ffa4955 != nullptr) && (parameter_4ffa4955Proxy != nullptr));
            }

            uint32_t result = implementation->Register(_parameter_4ffa4955);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_4ffa4955Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_4ffa4955Proxy, message->Response());
            }
        },
        ...
        [](Core::ProxyType<Core::IPCChannel>& channel, Core::ProxyType<RPC::InvokeMessage>& message) {
            ...
            RPC::Data::Frame::Reader reader(message->Parameters().Reader());
            const Core::instance_id parameter_76c07941Implementation = reader.Number<Core::instance_id>();

            ICompositPlugin::INotification* _parameter_76c07941 = nullptr;
            ProxyStub::UnknownProxy* parameter_76c07941Proxy = nullptr;
            if (parameter_76c07941Implementation != 0) {
                parameter_76c07941Proxy = RPC::Administrator::Instance().ProxyInstance(channel, parameter_76c07941Implementation, false, _parameter_76c07941);

                ASSERT((_parameter_76c07941 != nullptr) && (parameter_76c07941Proxy != nullptr));
            }

            uint32_t result = implementation->Unregister(_parameter_76c07941);

            RPC::Data::Frame::Writer writer(message->Response().Writer());
            writer.Number<uint32_t>(result);

            if (parameter_76c07941Proxy != nullptr) {
                RPC::Administrator::Instance().Release(parameter_76c07941Proxy, message->Response());
            }
        }
        , nullptr
    ...
    public:
        ...
        uint32_t Register(ICompositPlugin::INotification* _parameter_4ffa4955) override
        {
            IPCMessage message(BaseClass::Message(0));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_4ffa4955));
        ...
        uint32_t Unregister(ICompositPlugin::INotification* _parameter_76c07941) override
        {
            IPCMessage message(BaseClass::Message(1));

            RPC::Data::Frame::Writer writer(message->Parameters().Writer());
            writer.Number<Core::instance_id>(RPC::instance_cast(_parameter_76c07941));
